...
def write_report(results: Dict[str, Any], out_dir: str, make_plots: bool = True) -> None:
    ...
    # Significance tables are rendered inside results_markdown(); here we only
    # persist the machine-readable CSVs.
    for key, cols in (("head_to_head",
                       ["comparison", "mean_diff", "ci_lo", "ci_hi", "p_value",
                        "n_pairs"]),
                      ("significance_vs_reference",
                       ["policy", "mean_diff", "ci_lo", "ci_hi", "p_value",
                        "significant_05", "n_pairs"])):
        rows = results.get("control", {}).get(key)
        if rows:
            to_csv(rows, os.path.join(out_dir, "tables", f"{key}.csv"), cols)
